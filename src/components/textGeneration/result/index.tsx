...
const Result: FC<IResultProps> = ({
  ...
  useEffect(() => {
    if(controlStopResponding) setResponsingFalse();
  }, [controlStopResponding]);
  ...
  const handleFeedback = async (feedback: Feedbacktype) => {
    const res = await updateFeedbackNew({ url: `messages/${messageId}/feedbacks`, body: { rating: feedback.rating, user: 'abc-123', content: feedback.rating === 'dislike' ? dislinkText : null }});
      ...
      sendWorkflowMessage(
        data,
        {
          ...
          onWorkflowFinished: ({ data }) => {
            ...
            if(!data.outputs) setCompletionRes('');
            else if(Object.keys(data.outputs).length > 1) setCompletionRes(data.outputs);
            else setCompletionRes(data.outputs[Object.keys(data.outputs)[0]]);
            setResponsingFalse();
  ...
  const renderTextGenerationRes = () => (
    <TextGenerationRes
      ...
      dislinkText={dislinkText}
      handleDislinkText={setDislinkText}
      feedback={feedback}
      isMobile={isMobile}
