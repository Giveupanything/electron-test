...
import { Loading, Clipboard, RefreshCcw01 } from '@/components/icon/communication';
import Toast from '@/components/base/toast';
import { updateFeedbackNew } from '@/api/text';
// import CodeEditor from '@/app/components/apply/textGeneration/result/workflow/code-editor';
// import WorkflowProcessItem from '@/app/components/apply/textGeneration/result/workflow/workflow-process';
import { type Feedbacktype, WorkflowProcess } from '@/types/app';
import { Popover } from 'antd';
import AudioBtn from '@/components/base/audio-btn';
import { useChatContext } from '@/components/base/audio-btn/context';
...
export type IGenerationItemProps = {
  ...
  taskId?: string
  dislinkText?: string
  handleDislinkText?: (text: string) => void
}
...
const GenerationItem: FC<IGenerationItemProps> = ({
  ...
  const handleFeedback = async (childFeedback: Feedbacktype) => {
    const res = await updateFeedbackNew({ url: `messages/${childMessageId}/feedbacks`, body: { rating: childFeedback.rating, content: dislinkText }});
  ...
  const [isQuerying] = useBoolean(false);
  ...
  // const [dislinkText, setDislinkText] = useState('');
  const dislinkDom = () => {
    return <>
      <div
        className="flex items-center p-1 h-8 bg-components-input-bg-active border border-components-input-border-active rounded-lg shadow-xs"
      >
        <input
          ...
          value={dislinkText}
          onChange={(e) => {
            handleDislinkText(e.target.value);
          }}
        />
        <button
          className="file-button"
          disabled={!dislinkText }
          onClick={() => {
            onFeedback?.({
              rating: 'dislike'
            });
  ...
  return (
    <div
      className={cn(className, isTop ? `rounded-xl border ${!isError ? 'border-gray-200 bg-white' : 'border-[#FECDCA] bg-[#FEF3F2]'} ` : 'rounded-br-xl !mt-0')}
      style={isTop
        ? {
              ...
              <div className="flex items-center">
                ...
                {isInWebApp && (
                  <>
                    ...
                    {!isWorkflow && !isError && messageId && <div className="mx-3 w-[1px] h-[14px] bg-gray-200" />}
                    {!isWorkflow && !isError && messageId && !feedback?.rating && (
                      <SimpleBtn className="!px-0">
                        <>
                          ...
                          <Popover content={dislinkDom} trigger="click">
                            <div className="flex w-6 h-6 items-center justify-center rounded-md cursor-pointer hover:bg-gray-100">
                    ...
                    {!isWorkflow && !isError && messageId && feedback?.rating === 'like' && (
                      <div
                        ...
                        className="flex w-7 h-7 items-center justify-center rounded-md cursor-pointer  !text-blue-600 border border-blue-200 bg-blue-100 hover:border-blue-300 hover:bg-blue-200"
                      >
                        <HandThumbUpIcon width={16} height={16} />
                      </div>
                    )}
                ...
                {isShowTextToSpeech && (
                  <>
                    <div className="ml-2 mr-2 h-[14px] w-[1px] bg-gray-200" />
                    <AudioBtn
                      id={messageId!}
                      className={'mr-1'}
