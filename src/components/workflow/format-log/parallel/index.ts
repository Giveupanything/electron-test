import { BlockEnum, type NodeTracing } from '@/types/app';
...
function addTitle({
  ...
  list.forEach((node) => {
    ...
    if(isParallelStartNode) {
      ...
      node.parallelDetail!.parallelTitle = `并行-${parallelIndexInfo}`;
    }
    ...
    if(isBrachStartNode) {
      ...
      node.parallelDetail!.branchTitle = `分支-${belongParallelIndexInfo}-${branchLetter}`;
    }
