...
import { Loading, File02, RefreshCcw01, Bookmark } from '@/components/icon/communication';
import Toast from '@/components/base/toast';
import { updateFeedback } from '@/api/text';
// import CodeEditor from '@/app/components/apply/textGeneration/result/workflow/code-editor';
import WorkflowProcessItem from '@/components/workflow/workflow-process';
import { type Feedbacktype, WorkflowProcess } from '@/types/app';
import ResultTab from '@/components/workflow/text-generate/item/result-tab';
import { fetchTextGenerationMessage } from '@/api/text';
...
const GenerationItem: FC<IGenerationItemProps> = ({
  ...
  const handleFeedback = async (childFeedback: Feedbacktype) => {
    const res = await updateFeedback({ url: `messages/${childMessageId}/feedbacks`, body: { rating: childFeedback.rating }});
          ...
          >
            ...
            <div className="flex">
              <div className="grow w-0">
                {workflowProcessData && (
                  <WorkflowProcessItem hideInfo data={workflowProcessData} expand={workflowProcessData.expand} />
                )}
                ...
                {isInWebApp && (
                  <>
                    ...
                    {!isError && messageId && !isWorkflow && (
                      <div className="mx-3 w-[1px] h-[14px] bg-gray-200" />
                    )}
