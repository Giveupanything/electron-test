...
import { NodeRunningStatus } from '@/types/app';
import TracingPanel from '../tracing-panel';
// import { Iteration } from '@/app/components/base/icons/src/vender/workflow';
import cn from '@/utils/classnames';
import type { IterationDurationMap, NodeTracing } from '@/types/app';
...
const IterationResultPanel: FC<Props> = ({
  ...
  const iterationStatusShow = (index: number, iteration: NodeTracing[], iterDurationMap?: IterationDurationMap) => {
    ...
    if(hasFailed) return <RiErrorWarningLine className="w-4 h-4 text-text-destructive" />;

    if(isRunning) return <RiLoader2Line className="w-3.5 h-3.5 text-primary-600 animate-spin" />;
