...
// import { Iteration } from '@/components/icon/communication';
...
const IterationLogTrigger = ({
  ...
  const getCount = (iteration_curr_length: number | undefined, iteration_length: number) => {
    if((iteration_curr_length && iteration_curr_length < iteration_length) || !iteration_length) return iteration_curr_length;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-left system-sm-medium text-components-button-tertiary-text">{`${getCount(nodeInfo.details?.length, nodeInfo.metadata?.iterator_length)}个迭代}` }{getErrorCount(nodeInfo.details) > 0 && (
        <>
          ，
          {`${getErrorCount(nodeInfo.details)}个失败`}
        </>
      )}</div>
