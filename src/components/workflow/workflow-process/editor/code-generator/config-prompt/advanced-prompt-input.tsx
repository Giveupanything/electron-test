...
import type { PromptRole, PromptVariable } from '@/types/app';
import {
  Clipboard,
  ClipboardCheck
} from '@/components/icon/communication';
import Tooltip from '@/components/base/tooltip';
import PromptEditor from '@/components/base/prompt-editor';
import ConfigContext from '../context/debug-configuration';
import { getNewVar, getVars } from '@/utils';
import { AppType } from '@/types/app';
import { useModalContext } from '@/context/modal-context';
import type { ExternalDataTool } from '@/types/app';
import { useToastContext } from '@/components/base/toast';
import { useEventEmitterContextContext } from '@/context/event-emitter';
import { createCommand } from 'lexical';
const ADD_EXTERNAL_DATA_TOOL = 'ADD_EXTERNAL_DATA_TOOL';
const INSERT_VARIABLE_VALUE_BLOCK_COMMAND = createCommand('INSERT_VARIABLE_VALUE_BLOCK_COMMAND');
            ...
            <div className={cn(s.boxHeader, 'flex justify-between items-center h-11 pt-2 pr-3 pb-1 pl-4 rounded-tl-xl rounded-tr-xl bg-white hover:shadow-xs')}>
              ...
              <div className={cn(s.optionWrap, 'items-center space-x-1')}>
                ...
                {!isCopied
                  ? (
                    <Clipboard
                      className="h-6 w-6 p-1 text-gray-500 cursor-pointer"
                      onClick={() => {
                        copy(value);
                        setIsCopied(true);
                      }}
