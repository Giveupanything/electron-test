...
const ComponentPicker = ({
  ...
  const handleSelectWorkflowVariable = useCallback((variables: string[]) => {
    ...
    if(variables[1] === 'sys.query' || variables[1] === 'sys.files') editor.dispatchCommand(INSERT_WORKFLOW_VARIABLE_BLOCK_COMMAND, [variables[1]]);
                    ...
                    <Fragment key={option.key}>
                      {
                        ...
                        index !== 0 && options.at(index - 1)?.group !== option.group && (
                          <div className="h-px bg-divider-subtle my-1 w-full -translate-x-1" />
                        )
                      }
