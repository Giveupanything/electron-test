...
import FileLinkLocal from '../file-from-link-or-local/file-link-local';
...
import type { FileUpload } from '@/types/app';
...
const FileUploaderInAttachment = ({
  ...
  const renderButton = useCallback((option: Option, open?: boolean) => {
    return (
      <Button
        key={option.value}
        className={cn('grow relative text-xs', open && 'bg-components-button-tertiary-bg-hover')}
  ...
  const renderOption = useCallback((option: Option) => {
    ...
    if(option.value === TransferMethod.remote_url && fileConfig?.allowed_file_upload_methods?.includes(TransferMethod.remote_url)) {
      return (
        <FileLinkLocal fileConfig={fileConfig} />
      );
    }
