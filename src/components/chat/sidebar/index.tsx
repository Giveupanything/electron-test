...
export default function Sidebar({ apiKey, fold }: Props) {
  const {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      await delConversation(showConfirm?.id || "", {
        apiKey,
  ...
  const handleRename = useCallback(
    async (name: string) => {
      ...
      try {
        setState({ loading: true });
        await renameConversation(showRename?.id || "", {
          apiKey,
          name,
          user: DEFAULT_USER_ID,
        });
  ...
  return (
    <div
      className={classNames(
        "w-1/6 max-w-[200px] bg-white border-r border-r-[rgba(0,0,0,0.1)] min-w-[135px] relative duration-500",
        {
          "!w-0 !min-w-0 overflow-hidden opacity-0": !fold,
        }
      )}
    >
      {/* bg-[#f7f9fb] */}
