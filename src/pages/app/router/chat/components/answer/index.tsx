import { FeedbackFunc, MessageRating } from '@/axios/type';
import { FC, memo, useState } from 'react';
import copy from 'copy-to-clipboard';

import { randomString } from '@/utils';
import Tooltip from '@/components/base/tooltip';
import { VisionFile } from '@/typings/app';
import s from './style.module.css';
import { Markdown } from '@/components/base/markdown';
import ImageGallery from '@/components/base/image-gallery';
import Citation from './citation';
import { debounce } from 'lodash-es';
// import Refresh from '@/components/base/icon/Refresh';
import ThumbsDown from '@/components/base/icon/ThumbsDown';
import ThumbsUp from '@/components/base/icon/ThumbsUp';
import ChevronRight from '@/components/base/icon/ChevronRight';
import LoadingAnim from '@/components/chat/answer/loading-anim';
...
export const OpeningStatementIcon: FC<{ className?: string }> = () => (
...
function Answer({
  ...
  switchSibling
    ...
    onRegenerate: (item: Chat.ChatItem) => void,
    switchSibling?: (id: string) => void
  }) {

  const { id, content, feedback, agent_thoughts, workflowProcess, citation } = item;
  ...
  const defaultOperation = () => {
    ...
    const userOperation = () => (
      <>
        <Tooltip selector={`user-feedback-${randomString(16)}`} content={isCopied ? '已复制' : '复制'}>
          {OperationBtn({
        ...
        {/* <Tooltip selector={`user-feedback-${randomString(16)}`} content="重新生成">
          {OperationBtn({
            innerContent: <IconWrapper>
              <Refresh className="p-[3.5px] w-6 h-6" />
 ...
 */
  const renderFeedbackRating = (rating: MessageRating | undefined) => {
    ...
    const ratingIconClassname = isLike ? 'text-[#305edf] bg-[#eef5fe] hover:bg-[#eef5fe]' : 'text-red-600 bg-red-100 hover:bg-red-200';
  ...
  const agentModeAnswer = (
    <div>
      {agent_thoughts?.map((item, index) => {
        ...
        const images = getImgs(item.message_files).map(item => item.url);
  ...
  return (
    <div key={id} className="mb-[24px]">
      <div className="flex items-start">
        ...
        <div className={`${s.answerWrap} chat-answer-container max-w-[calc(100%-50px)]`}>
          <div className={`${s.answer} relative text-sm text-gray-900`}>
            <div
              className={`ml-2 py-3 px-4 rounded-tr-2xl rounded-b-2xl ${workflowProcess && 'min-w-[480px]'}`}
              style={{
                backgroundImage: 'linear-gradient(180deg,#fff,hsla(0,0%,100%,.6))'
              }}
            >
              {/* {workflowProcess && (
